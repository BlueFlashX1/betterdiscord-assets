...
module.exports = class SoloLevelingToasts {
  ...
  loadSettings() {
    try {
      this.settings = _sharedLoadSettings("SoloLevelingToasts", this.defaultSettings);
  ...
  saveSettings() {
    try {
      _sharedSaveSettings("SoloLevelingToasts", this.settings);
  ...
  _buildAndMountCardToast(opts) {
    ...
    // Dedup check (text-based) — still useful for content-identical toasts
    ...
    if (replaceKey) toast._cardReplaceKey = replaceKey;
