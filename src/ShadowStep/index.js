...
module.exports = class ShadowStep {
  ...
  initWebpack() {
    const { Webpack } = BdApi;
    this._ChannelStore = Webpack.getStore("ChannelStore") ?? null;
    if (!this._ChannelStore) this.debugError("Webpack", "ChannelStore not found");
    this._GuildStore = Webpack.getStore("GuildStore") ?? null;
    if (!this._GuildStore) this.debugError("Webpack", "GuildStore not found");
    this._SelectedGuildStore = Webpack.getStore("SelectedGuildStore") ?? null;
    if (!this._SelectedGuildStore) this.debugError("Webpack", "SelectedGuildStore not found");
  ...
  loadSettings() {
    try {
      if (typeof _PluginUtils?.loadSettings === "function") {
        ...
        this.settings = _sharedLoadSettings(PLUGIN_NAME, DEFAULT_SETTINGS);
      }
  ...
  saveSettings() {
    try {
      if (typeof _PluginUtils?.saveSettings === "function") {
        ...
        _sharedSaveSettings(PLUGIN_NAME, this.settings);
      }
    } catch (err) {
  ...
  removeAnchor(anchorId) {
    // Re-index sortOrder immutably
    this.settings.anchors = this.settings.anchors.filter((a) => a.id !== anchorId).map((a, i) => ({ ...a, sortOrder: i }));
    this._rebuildAnchorIndex();
    this.saveSettings();
    if (this._panelForceUpdate) this._panelForceUpdate();
  ...
  _pruneStaleAnchors() {
    ...
    if (pruned > 0) {
      this.settings.anchors = this.settings.anchors.map((a, i) => ({ ...a, sortOrder: i }));
      this._rebuildAnchorIndex();
      this.saveSettings();
    ...
    this.playTransition(() => {
      ...
      this._navigate(path, {
        ...
        onConfirmed: () => {
          ...
          // Stamp cooldown here — nav is confirmed, so cooldown only triggers on success
