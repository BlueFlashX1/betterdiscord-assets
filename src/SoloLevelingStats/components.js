function buildChatUIComponents(pluginInstance) {
  ...
  function StatButton({ statKey, onAllocate, totalStats, titleBonus, shadowBuffs }) {
    ...
    // STAT_METADATA may not have an entry for every statKey (migration drift,
    // custom stats added to STAT_KEYS but not METADATA, etc.). Fall back to
    // the upper-cased key so the button still renders instead of crashing.
    // Same defensive pattern used in the radar chart render code.
  ...
  // StatAllocation: Points display + allocation buttons.
  // STAT_KEYS mixes allocatable base stats (strength/agility/intelligence/
  // vitality/perception) with derived stats (attack/defense/critChance/
  // critDamage). Only the base stats have entries in STAT_METADATA, so we
  // filter the button list by metadata presence — same pattern the radar
  // chart uses. Derived stats are computed from the base 5 and must not
  // be user-allocatable.
