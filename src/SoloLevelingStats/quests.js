module.exports = {
  ...
  completeQuest(questId) {
    ...
    // Route XP through the governor system (NaN guards, level-up checks, save, shadow share)
  ...
  showQuestCompletionCelebration(questName, xpReward, statPoints) {
    try {
      ...
      // Pre-build quest element map to avoid per-quest querySelector on every tick
      const questElements = new Map();
      ...
      // Targeted DOM updates instead of full innerHTML rebuild.
      // Each celebration owns its own interval so concurrent completions
      // don't cross-cancel; stop() iterates _questCelebrations to clean up.
