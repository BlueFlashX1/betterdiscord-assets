module.exports = {
  ...
  _initializeLoadedSettings(saved) {
    ...
    // Always reconcile level/xp against totalXP. Without this, a persisted
    // `level: 1` paired with a real `totalXP` (e.g. 3.8B) slips past the
    // guard above (level=1 is a valid number ≥ 1). Then the first message
    // captures `oldLevel = 1`, recomputes new level from totalXP → L1187,
    // fires a bogus "Level Up!" toast and runs natural-stat-growth ~1186
    // times as if the user just leveled up that much in one message.
    const derivedLevelInfo = this.getCurrentLevel();
    if (
      typeof derivedLevelInfo?.level === 'number' &&
      derivedLevelInfo.level !== this.settings.level
    ) {
  ...
  async _saveSettingsImmediate() {
    ...
    // Coalesce concurrent immediate-save requests. Startup fires several
    // saveSettings(true) calls in the same tick (rank promotion, retro
    // stat growth, file-source rescue, level reconciliation cascade) —
    // without this guard each spawns a separate IDB + BdApi.Data + backup
    // write, flooding the console and contending on writes for identical
    // payload. Subsequent callers await the same in-flight promise.
