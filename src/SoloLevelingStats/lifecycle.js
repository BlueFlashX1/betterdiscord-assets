...
module.exports = {
  ...
  async start() {
    ...
    try {
      ...
      // Initialize tracked timer registry for this session
      ...
      // Initialize equipment stat bonus integration
    ...
    //
    // SKIP for saves that already have real progress: transient stat values
    // during startup (channelsVisited Set freshly converted, timeActive not
    // yet ticked this session, etc.) can cause checkAchievementCondition to
    // spuriously fail for legitimately-earned achievements. Those then
    // re-pass the check on the first chat message and fire bogus
    // "ACHIEVEMENTS UNLOCKED" toasts. Fresh installs (no real progress) still
    // run revalidation so a corrupted seed save can self-heal.
