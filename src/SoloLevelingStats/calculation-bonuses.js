module.exports = {
  ...
  calculateActivityStreakBonus() {
    ...
    // NOTE: cacheKey is intentionally built AFTER the streak increment block
    // below. Building it here (with the pre-increment value) caused a stale
    // cache hit: the first message of a new day would increment streakDays but
    // the cache key still encoded the old value, so the updated bonus was never
    // stored and every subsequent call returned the wrong (lower) bonus.
    ...
    try {
      ...
      // Build cache key here, after the streak has been updated, so it encodes
      // the correct (post-increment) streakDays value.
