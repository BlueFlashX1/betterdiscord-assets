...
function loadSettings(plugin, defaultSettings, { pluginId }) {
  ...
  const score = (candidate) => {
    const ts = candidate.data?._metadata?.lastSave ? new Date(candidate.data._metadata.lastSave).getTime() || 0 : 0;
    const wpCount = Array.isArray(candidate.data?.waypoints) ? candidate.data.waypoints.length : 0;
    return ts * 1000 + wpCount;  // recency primary, count secondary
  };
