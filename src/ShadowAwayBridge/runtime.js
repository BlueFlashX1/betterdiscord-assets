...
`;
      ...
      _createHeaderWidget() {
        ...
        icon.addEventListener("mouseenter", () => showToolbarTooltip(icon, "sl-toolbar-tip-sab", "Shadow Away Report"));
        icon.addEventListener("mouseleave", () => hideToolbarTooltip("sl-toolbar-tip-sab"));
        return icon;
      }
      ...
      _teardownHeaderWidget() {
        ...
        if (icon) icon.remove();
        removeToolbarTooltip("sl-toolbar-tip-sab");
        this._headerWidgetBusy = false;
      }
      ...
      async _consumePendingDigestForSelectedContext() {
        ...
        if (!myUserId) {
          ...
          this._showShadowReportStatusModal(
            "Shadow Away Report",
            "Unable to identify your account. If this persists, try reloading Discord."
          );
          return false;
        }
        const { guildId, channelId } = this._getSelectedContext();
        ...
        if (!outcome.consumed) {
          ...
          if (outcome.reason === "locked_to_channel") {
            ...
            this._showShadowReportStatusModal(
              "Shadow Away Report",
              `This report is locked to ${expected}. Open that channel and click the Shadow Away icon.`
            );
            return false;
          }
