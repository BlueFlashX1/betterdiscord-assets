...
function handleMembersHover(ctx, runtime) {
  if (ctx.isPanelGated?.("members")) { clearPanelHoverState(ctx, "members"); return; }
...
function handleProfileHover(ctx, runtime) {
  if (ctx.isPanelGated?.("profile")) { clearPanelHoverState(ctx, "profile"); return; }
...
export function injectToolbarIcon(ctx) {
  ...
  if (!icon) {
    ...
    ensureTooltipCSS();
    icon.addEventListener("mouseenter", () => showToolbarTooltip(icon, "sl-toolbar-tip-ra", "Ruler's Authority"), iconSignal);
    icon.addEventListener("mouseleave", () => hideToolbarTooltip("sl-toolbar-tip-ra"), iconSignal);
  }
...
export function teardownToolbarObserver(ctx) {
  ...
  removeToolbarTooltip("sl-toolbar-tip-ra");
}
