...
const _safeGetByKeys = (...keys) => {
  try { return BdApi.Webpack.getByKeys(...keys) || null; }
  catch (_) { return null; }
};
...
const _createModules = () => ({
  ...
  get members()  { if (!this._membersResolved)  { this._members  = _safeGetByKeys("membersWrap", "hiddenMembers");               this._membersResolved = true; }  return this._members; },
  get sidebar()  { if (!this._sidebarResolved)  { this._sidebar  = _safeGetByKeys("sidebar", "activityPanel", "sidebarListRounded"); this._sidebarResolved = true; }  return this._sidebar; },
  get panel()    { if (!this._panelResolved)    { this._panel    = _safeGetByKeys("outer", "inner", "overlay");                   this._panelResolved = true; }    return this._panel; },
  get search()   { if (!this._searchResolved)   { this._search   = _safeGetByKeys("searchResultsWrap", "stillIndexing", "noResults"); this._searchResolved = true; }   return this._search; },
  get toolbar()  { if (!this._toolbarResolved)  { this._toolbar  = _safeGetByKeys("updateIconForeground", "search", "downloadArrow"); this._toolbarResolved = true; }  return this._toolbar; },
  get icons()    { if (!this._iconsResolved)    { this._icons    = _safeGetByKeys("selected", "iconWrapper", "clickable", "icon");    this._iconsResolved = true; }    return this._icons; },
  get guilds()   { if (!this._guildsResolved)   { this._guilds   = _safeGetByKeys("chatContent", "noChat", "parentChannelName");      this._guildsResolved = true; }   return this._guilds; },
  get channels() { if (!this._channelsResolved) { this._channels = _safeGetByKeys("channel", "closeIcon", "dm");                     this._channelsResolved = true; } return this._channels; },
});
...
const { setPluginUtils, isEditableTarget, matchesHotkey } = require("./hotkeys");
