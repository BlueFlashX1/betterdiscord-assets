...
module.exports = class SkillTree {
  ...
  start() {
    ...
    // Recalculate SP on startup based on current level — MUST run before setupLevelUpWatcher
    // so the flag is set before checkForLevelUp() fires inside setupLevelUpWatcher.
    this.recalculateSPFromLevel();
    // Flag consumed by setupLevelUpWatcher → checkForLevelUp to prevent double SP award.
  ...
  setupLevelUpWatcher() {
    ...
    // Skip if recalculateSPFromLevel() already ran during start() — it handles the
    // same level-gap logic and we must not double-award SP for the same levels.
  ...
  loadSettings() {
    try {
      ...
      // Legacy unlockedSkills array migration (pre-skillLevels format)
