...
// counts rarely change, so a 5-minute TTL is safe and dramatically reduces
// the cost of the 15-second refresh interval. See refreshGuildIconHints below
// for the rationale.
const GUILD_HINT_CACHE_TTL_MS = 5 * 60 * 1000;
...
function applyGuildHintTitle(plugin, node, guildId, guild, stats) {
  ...
  // PERF (2026-04-12): timestamp cache entries so refreshGuildIconHints can
  // skip the entire getGuildHintStats call when the cache is fresh.
...
function refreshGuildIconHints(plugin, snowflakeRegex) {
  ...
  for (const node of nodes) {
    ...
    // PERF (2026-04-12): Check cache freshness BEFORE calling getGuildHintStats.
    //
    // Prior code called getGuildHintStats unconditionally each cycle. That
    // function calls Discord's _GuildMemberCountStore.getMemberCount(guildId)
    // and getOnlineCount(...) for each guild. For guilds whose data isn't in
    // Discord's in-memory cache, those store calls trigger lazy IDB loads in
    // Discord's main process.
    //
    // For users with many guilds (50+), this caused recurring ~5-second
    // bursts of ~270 file opens/sec on Discord's IndexedDB every 15 seconds —
    // visible in fs_usage as ~125k pread calls/sec on
    // https_discord.com_0.indexeddb.leveldb. Discord main process pegged at
    // 100-150% CPU during each burst, even though the renderer's JS work was
    // tiny (it just queues the store calls; main processes the IDB reads).
    //
    // The 5-minute TTL is safe because guild member counts barely change in
    // that timeframe, and the data is only used for hover tooltips on the
    // guild list anyway.
    const cached = plugin._guildHintCache.get(guildId);
    ...
    if (cacheFresh && domSynced) {
      // Cache is fresh AND DOM marker is current — nothing to do.
      continue;
    }

    if (cacheFresh && !domSynced) {
      // Cache is fresh but DOM lost the marker (React re-render) — re-apply
      // the cached value WITHOUT re-querying Discord's stores.
      applyGuildHintTitle(plugin, node, guildId, guild, cached);
      continue;
    }

    // Cache is stale or missing — fetch fresh stats. This is the only path
    // that triggers Discord store/IDB activity, and it now runs at most once
    // per guild per TTL window instead of every 15 seconds.
