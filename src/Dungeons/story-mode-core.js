...
module.exports = {
  ...
  async exitDemonCastle() {
    ...
    if (dungeon) {
      ...
      const pendingEssence = dungeon?._pendingEssence || 0;
  ...
  _completeDemonCastleFloor(channelKey, dungeon, reason) {
    ...
    // Emit essence for the floor — only flush the residual pending count.
    // Kills already flushed during combat (every 10 kills in difficulty-contributions.js)
    // have already been awarded; emitting the total here would double-count them.
