...
module.exports = {
  ...
  async fixShadowBaseStatsToRankBaselines() {
    try {
      ...
      // Persist settings synchronously before marking the migration complete.
      // this.saveSettings() is debounced and not awaited — if the plugin is
      // unloaded between the debounced write and the flag save, the flag would
      // be set but the data would not be persisted, causing the migration to
      // appear complete on next load while the changes are absent.
