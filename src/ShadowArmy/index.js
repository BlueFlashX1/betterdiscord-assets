...
const ShadowArmy = class ShadowArmy {
  ...
  async start() {
    ...
    // Sync current essence to ItemVault — but only if ItemVault hasn't already
    // received live events (e.g. from Dungeons mob kills that started before us).
    // Using `set` when ItemVault already has a higher balance would stomp
    // essence earned between Dungeons startup and ShadowArmy startup.
    try {
      const essence = this.settings?.shadowEssence?.essence || 0;
      if (essence > 0) {
        const currentVaultBalance = window.ItemVault?.getBalance?.('shadow_essence') ?? 0;
    ...
    // Listen for Dungeons essence awards via shared event bus
    {
      ...
      this._dungeonEssenceListener = (data) => {
        ...
        if (essenceGain > 0) {
          ...
          // Mirror to ItemVault — but NOT for mob_kill, which difficulty-contributions.js
          // already mirrors directly to avoid double-adding essence for the 10-kill flush path.
  ...
  async stop() {
    ...
    // Flush any pending debounced save (fire-and-forget — BD does NOT await stop()
    // so any awaited work here would race with the next instance's start().)
