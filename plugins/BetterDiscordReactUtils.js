...
function _logMainContentMissingOnce() {
  // Each plugin bundles/loads its own copy of this helper, so a module-local
  // flag fired the warning once PER PLUGIN (~5x at startup). Use a global flag
  // so the identical warning prints once for the whole session.
  if (_mainContentMissingWarned || globalThis.__BD_RU_MAINCONTENT_WARNED) return;
  _mainContentMissingWarned = true;
  globalThis.__BD_RU_MAINCONTENT_WARNED = true;
  console.warn(
