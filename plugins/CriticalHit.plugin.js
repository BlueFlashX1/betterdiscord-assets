  ...
  applyCritStyle(messageElement) {
    ...
    try {
      ...
      try {
        ...
        // Inject per-message CSS targeting [data-message-id] for re-render persistence
        const _applyCritMsgId = this.getMessageIdentifier(actualMessageElement);
        if (_applyCritMsgId && !_applyCritMsgId.startsWith('hash_')) {
          this.injectCritMessageCSS(_applyCritMsgId, {
            gradient: this.settings.critGradient !== false,
