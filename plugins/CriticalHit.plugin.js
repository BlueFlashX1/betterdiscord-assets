 ...
 * - NEW FEATURES: Added Speedy Space Goat Oddity font for Shadow Arise animation text
...
module.exports = class CriticalHit {
  ...
  // DEFAULT_EXCLUDED_PROPERTIES, createGradientStyles — REMOVED in v3.4.0
  ...
  // getExpectedCritVisuals, normalizeColorForCompare, hasSolidColorMismatch,
  // stampCritVisualMetadata, excludeHeaderElements
  // — REMOVED in v3.4.0 (per-message CSS handles all styling, no inline style verification needed)

  // verifyGradientStyles, applyGradientStyles — REMOVED in v3.4.0
  ...
  // verifyGradientApplied — REMOVED in v3.4.0
  ...
  // applyGradientStylesWithSettings, applySolidColorStyles, applyFontStyles, applyGlowEffect
  // — REMOVED in v3.4.0 (per-message CSS handles all styling)
  ...
  buildMessageCSSRule(messageId, critSettings = {}) {
    ...
    const gradientColor = critSettings.gradientColor || this.settings.critGradientColor || this.DEFAULT_GRADIENT_COLORS;
...
${childSel} {
  ...
  // setupGradientMonitoring() — REMOVED in v3.4.0 (per-message CSS handles persistence via [data-message-id])
  // setupGradientRestorationRetryObserver() — REMOVED in v3.4.0 (per-message CSS handles persistence)
  ...
  get DEFAULT_GRADIENT_COLORS() {
    return 'linear-gradient(to right, #8a2be2 0%, #9b30ff 15%, #7b21c6 30%, #8a2be2 50%, #9b30ff 70%, #7b21c6 85%, #8a2be2 100%)';
  }
  ...
  // getCritContentCandidates, ensureCritTextContentClass, applyGradientToContentForStyling,
  // applySolidColorToContentForStyling, applyGlowToContentForStyling
  // — REMOVED in v3.4.0 (per-message CSS handles all styling)
  ...
  get DEFAULT_ANIMATION_FONT() {
    return 'Speedy Space Goat Oddity';
  }
  ...
  get LOCAL_ONLY_FONTS() {
    return ['friend or foe', 'friend or foe bb', 'speedy space goat', 'speedy goat'];
  }
  ...
  _getPluginsFolderFromBdApi() {
    if (typeof BdApi !== 'undefined' && BdApi.Plugins?.folder) {
      ...
      // BetterDiscord returns absolute path, ensure it ends with /
  ...
  loadLocalFont(fontName, fontFamily = null) {
    ...
    try {
      ...
      if (fontName.toLowerCase().includes('friend or foe')) {
        ...
        fontFileName = 'SpeedySpaceGoatOddity'; // Exact filename match (for Shadow Arise animation)
      }

      // Get the actual file path (not file:// URL)
      ...
      // BetterDiscord plugins run in sandboxed renderer process
      // file:// URLs are blocked in CSS @font-face
      // Use embedded base64 data URIs for font loading
      let fontSrc = '';

      // Map font names to embedded data getters
      const fontDataMap = {
        FriendorFoeBB: {
          woff2: () => this.FRIENDORFOEBB_WOFF2_DATA,
        },
        SpeedySpaceGoatOddity: {
          woff2: () => this.SPEEDYSPACEGOATODDITY_WOFF2_DATA,
        },
      };
      ...
      if (document.fonts && document.fonts.check) {
        ...
        document.fonts.ready
          ...
          .catch((fontError) => {
            ...
            // Font will fall back to system fonts if loading fails
      ...
      // Graceful error handling - font loading failures are non-critical
  ...
  loadGoogleFont(fontName) {
    ...
    try {
      ...
      // Add error handler for link loading failures
      fontLink.onerror = () => {
  ...
  // FONT_CHECK_THROTTLE_MS — REMOVED in v3.4.0 (per-message CSS handles font enforcement)
  ...
  injectAnimationCSS() {
    ...
    // Always load animation font (even if same as crit font, it might be used differently)
  ...
  // _applyCritFontToElement() — REMOVED in v3.4.0 (per-message CSS handles font via [data-message-id] selector)
  // _needsFontApplication() — REMOVED in v3.4.0 (per-message CSS handles font enforcement)

  /**
   * Injects CSS styles for crit messages (animation keyframes, position)
   * Pre-loads crit font. Gradient/font/glow handled by per-message CSS rules.
   */
  injectCritCSS() {
  ...
  /**
   * Helper: Loads animation font for Shadow Arise floating text animation
   * Uses Speedy Space Goat Oddity (requires local files, not on Google Fonts)
   * @param {string} fontName - Name of the font to load (default: Speedy Space Goat Oddity)
   * @returns {boolean} True if font was loaded
   */
  loadCritAnimationFont(fontName = null) {
    ...
    if (isSpeedyGoat) {
      // Speedy Space Goat Oddity is not on Google Fonts (used by Shadow Arise animation)
      if (this.settings.useLocalFonts) {
        const loaded = this.loadLocalFont(fontToLoad);
        if (loaded) return true;
      }
  ...
  debugLog(operation, message, data = null) {
    ...
    // Only log if debug mode is enabled in settings (toggleable)
    if (this.settings?.debugMode === true) {
  ...
  debugError(operation, error, context = {}) {
    ...
    // Only log errors if debug mode is enabled in settings (toggleable)
    if (this.settings?.debugMode === true) {
