...
module.exports = class Dungeons {
  ...
  async start() {
    ...
    console.log('Dungeons plugin started');
  }
  ...
  loadPluginReferences() {
    try {
      ...
      if (soloPlugin?.instance) {
        ...
        console.log('[Dungeons] ✓ SoloLevelingStats plugin loaded');
        ...
        console.log('[Dungeons] ⚠ SoloLevelingStats plugin not found');
      }
      ...
      if (shadowPlugin?.instance) {
        ...
        console.log('[Dungeons] ✓ ShadowArmy plugin loaded');
      } else {
        console.log('[Dungeons] ⚠ ShadowArmy plugin not found');
      }
      ...
      if (toastsPlugin) {
        if (toastsPlugin.instance) {
          ...
          if (typeof toastsPlugin.instance.showToast === 'function') {
            ...
            console.log('[Dungeons] ✓ SoloLevelingToasts plugin loaded successfully');
          } else {
            console.warn(
              '[Dungeons] ⚠ SoloLevelingToasts plugin instance found but showToast method missing'
            );
          }
        } else {
          console.warn(
            `[Dungeons] ⚠ SoloLevelingToasts plugin exists but instance not ready (enabled: ${toastsPlugin.enabled})`
          );
        }
      } else {
        console.warn(
          '[Dungeons] ⚠ SoloLevelingToasts plugin not found - will use fallback notifications'
        );
      }
    } catch (error) {
      ...
      mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node) => {
          ...
          const messageElement =
            ...
            (node.classList?.contains('message') ? node : null);
          if (messageElement) this.handleMessage(messageElement);
        });
      });
    });
  ...
  async handleMessage(messageElement) {
    ...
    try {
      if (!this.isUserMessage(messageElement)) return;
  ...
  async checkDungeonSpawn(channelKey, channelInfo) {
    if (this.activeDungeons.has(channelKey)) return;
    const lastSpawn = this.settings.lastSpawnTime[channelKey] || 0;
    if (Date.now() - lastSpawn < this.settings.dungeonDuration) return;
    if (Math.random() * 100 > this.settings.spawnChance) return;
  ...
  async createDungeon(channelKey, channelInfo, rank) {
    ...
    if (this.storageManager) {
      try {
        ...
        console.error('Dungeons: Failed to save dungeon to IndexedDB', error);
      }
    }

    this.saveSettings();
    this.showDungeonIndicator(channelKey, channelInfo);
    this.showToast(`⚔️ ${dungeonName} Spawned!`, 'info');
    ...
    console.log(`Dungeons: Created ${rank}-rank dungeon: ${dungeonName}`);
  }
  ...
  async selectDungeon(channelKey) {
    ...
    this.updateUserHPBar();
    this.showToast(`⚔️ Joined ${dungeon.name}!`, 'info');
    this.saveSettings();
    this.closeDungeonModal();
  }
  ...
  async completeDungeon(channelKey, reason) {
    ...
    if (reason === 'complete') {
      this.showToast(`🎉 ${dungeon.name} Cleared!`, 'success');
    } else if (reason === 'boss') {
      this.showToast(`👑 Boss Defeated! ${dungeon.name} Cleared!`, 'success');
  ...
  async grantShadowDungeonXP(channelKey, dungeon) {
    ...
    if (totalXPGranted > 0) {
      ...
      this.showToast(`✨ Shadows gained ${totalXPGranted} XP from ${dungeon.name}!`, 'success');
    }

    if (leveledUpShadows.length > 0) {
      for (const { shadow, levelBefore, levelAfter } of leveledUpShadows) {
        this.showToast(
          `⭐ ${shadow.roleName || 'Shadow'} (${
            shadow.rank
      ...
      if (currentDungeons.length === 0) {
        ...
        currentDungeons.forEach((dungeon) => {
          ...
          listHTML +=
            ...
            '</div>';
          listHTML +=
            '<div style="color: #f59e0b; font-size: 11px;">⏰ ' + timeStr + ' remaining</div>';
          listHTML += '</div>';
          if (isActive) {
            listHTML +=
  ...
  showMobKillSummary(channelKey) {
    ...
    this.showToast(`⚔️ Killed ${count} mob${count !== 1 ? 's' : ''} in ${dungeon.name}`, 'info');
    this.saveSettings();
  }
