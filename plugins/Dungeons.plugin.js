...
// =============================================================================
// MAJOR SECTION INDEX (navigation map for this file)
// =============================================================================
// 1) Module bootstrap, shared helpers, and persistence layer
// 2) Plugin bootstrap, lifecycle, settings, and integration wiring
// 3) Spawn pipeline (observer -> dungeon creation -> mob wave generation)
// 4) Player flow and deployment (join/select/deploy/rebalance/allocation)
// 5) Combat core (stats/rank models, shadow/boss/mob attack processors)
// 6) Post-combat systems (resurrection, completion, ARISE, XP grants)
// 7) UI and rendering systems (indicators, HP bars, CSS, notifications)
// 8) Performance/runtime maintenance (visibility simulation, restore, GC)
// =============================================================================

// =============================================================================
// MAJOR SECTION: MODULE BOOTSTRAP, SHARED HELPERS, AND STORAGE PRIMITIVES
// =============================================================================
...
// =============================================================================
// MAJOR SECTION: PERSISTENCE LAYER (IndexedDB storage managers)
// =============================================================================
...
// Fallback: plain text with CSS styling if SVG fails to render
...
module.exports = class Dungeons {
  ...
  // =============================================================================
  // MAJOR SECTION: BOOTSTRAP, CACHES, LOOP INFRASTRUCTURE, AND LIFECYCLE
  // =============================================================================
  ...
  /** Settings, version, plugin name, debug config, shared utilities, rank scaling. */
  _initDefaults() {
  ...
  /** All interval/timeout IDs, retry timeouts, debounce timers. */
  _initTimers() {
  ...
  /** CacheManager, shadow count cache, boss bar cache, other caches. */
  _initCaches() {
  ...
  /** Combat state, dungeon state, flags, spawn state. */
  _initState() {
  ...
  /**
   * Build precomputed O(1) typed-array lookup tables for rank-based values.
  ...
  /**
   * Process a single dungeon's combat for one tick. Extracted from _combatLoopTick for parallel execution.
  ...
  async _processDungeonCombatTick(channelKey, dungeon, now, isWindowVisible, shadowBudget, mobBudget, dungeonIndex = 0) {
    try {
      ...
      if (this.shadowAttackIntervals.has(channelKey)) {
        ...
        const backgroundInterval = this._shadowBackgroundIntervalMs.get(channelKey) || 5000;
        ...
        if ((this._combatTickCount + dungeonIndex) % 10 === 0) {
          ...
          this.settings.debug && console.log(`[Dungeons] COMBAT_TRACE: Tick #${this._combatTickCount} — ch=${channelKey.slice(-8)}, shadows=${allocCount}, mobs=${mobCount}, bossHP=${bossHp}, elapsed=${elapsed}ms, willFire=${elapsed >= intervalTime}`);
        }
        ...
        if (elapsed >= intervalTime) {
          ...
          this.settings.debug && console.log(`[Dungeons] COMBAT_MOB_TRACE: ch=${channelKey.slice(-8)}, isActive=${isActive}, mobsBefore=${preAttackMobs}, mobsAfter=${postAttackMobs}, bossHP=${dungeon.boss?.hp}, elapsed=${elapsed}ms`);
      ...
      if (this.bossAttackTimers.has(channelKey) && dungeon.boss?.hp > 0) {
        ...
        const backgroundInterval = this._bossBackgroundIntervalMs.get(channelKey) || 5000;
        const intervalTime = isActive ? activeInterval : backgroundInterval;
      ...
      if (this.mobAttackTimers.has(channelKey)) {
        const activeInterval = 1000;
        const backgroundInterval = this._mobBackgroundIntervalMs.get(channelKey) || 5000;
        const intervalTime = isActive ? activeInterval : backgroundInterval;
  ...
  // =============================================================================
  // MAJOR SECTION: STATS, PLUGIN INTEGRATIONS, CHANNEL DISCOVERY, AND SPAWN ENTRY
  // =============================================================================
  ...
  // =============================================================================
  // MAJOR SECTION: PLAYER FLOW, DEPLOYMENT, REBALANCE, AND RANK ALLOCATION
  // =============================================================================
  ...
  // =============================================================================
  // MAJOR SECTION: COMBAT CORE (math models, helpers, and attack processing)
  // =============================================================================
   ...
   * Multiplier: 2.2–3.0× mob baseline to keep bosses significantly stronger.
  ...
  /**
   * Clamp a stat value within [min, max] with optional debug logging.
   * @param {number} value - Raw stat value
   * @param {number} min - Minimum allowed
   * @param {number} max - Maximum allowed
   * @param {string} label - Stat name for debug output
   ...
   */
  /**
   * Get cached exclusion sets (ShadowExchange + ShadowSenses) with 5s TTL.
   * Avoids redundant BdApi.Plugins.get() calls across multiple getCombatReadyShadows invocations per tick.
   */
  ...
  // =============================================================================
  // MAJOR SECTION: POST-COMBAT SYSTEMS (resurrection, completion, extraction, XP)
  // =============================================================================
  ...
  showAriseButton(channelKey) {
    ...
    // Skip if a valid button already exists
    const existing = document.querySelector(`[data-arise-button="${channelKey}"]`);
   ...
   *    - Grant XP to shadow
   * 4. Queue deferred post-processing for level/rank/growth updates and notifications
   */
  ...
  // =============================================================================
  // MAJOR SECTION: UI RENDERING, INDICATORS, HP BARS, AND DOM HELPERS
  // =============================================================================
  ...
  // =============================================================================
  // MAJOR SECTION: VISIBILITY/PERF SIMULATION, RESTORE PIPELINE, AND MAINTENANCE
  // =============================================================================
  ...
  injectCSS() {
    ...
    const cssContent = `
      ...
      .dungeon-boss-hp-container ~ [class*="toolbar"] [class*="comment"],
      .dungeon-boss-hp-container ~ [class*="toolbar"] [class*="thread"],
      .dungeon-boss-hp-container ~ [class*="toolbar"] [class*="reply"] {
