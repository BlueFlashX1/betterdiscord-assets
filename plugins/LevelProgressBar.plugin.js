...
module.exports = class LevelProgressBar {
  ...
  _getFileBackupPath() {
    var _a;
    return ((_a = SLUtils == null ? void 0 : SLUtils.getSoloLevelingBackupFilePath) == null ? void 0 : _a.call(SLUtils, "LevelProgressBar.json")) || null;
  }
  readFileBackup() {
    const filePath = this._getFileBackupPath();
    if (!filePath) return null;
    const readShared = SLUtils == null ? void 0 : SLUtils.readJsonFileSafe;
  ...
  writeFileBackup(data) {
    ...
    const writeShared = SLUtils == null ? void 0 : SLUtils.writeJsonFileSafe;
  ...
  _getCandidateTimestamp(data) {
    var _a;
    if (typeof (SLUtils == null ? void 0 : SLUtils.getSavedTimestampFromMetadata) === "function") {
  ...
  _pickBestSettingsCandidate(candidates) {
    if (typeof (SLUtils == null ? void 0 : SLUtils.pickNewestSettingsCandidate) === "function") {
  ...
  async loadSettings() {
    try {
      ...
      if (best == null ? void 0 : best.data) {
        this.debugLog("LOAD_SETTINGS", `Selected settings candidate`, {
          source: best.source,
          ts: best.ts ? new Date(best.ts).toISOString() : "none",
          candidateCount: candidates.length
        });
        this.settings = SLUtils ? SLUtils.mergeSettings(this.defaultSettings, best.data) : structuredClone({ ...this.defaultSettings, ...best.data });
      } else {
  ...
  async saveSettings() {
    try {
      ...
      cleanSettings._metadata = { lastSave: (/* @__PURE__ */ new Date()).toISOString(), version: "1.5.0" };
      if (this.saveManager) {
        try {
  ...
  startUpdating() {
    ...
    this.updateInterval = setInterval(() => {
      if (document.hidden) return;
      this.queueProgressBarUpdate();
    }, this.settings.updateInterval || 5e3);
    this.debugLog("START_UPDATE", "Fallback polling started", {
      interval: this.settings.updateInterval || 5e3
    });
  }
  stopUpdating() {
    if (this.updateInterval) {
