...
const _bdLoad = (f) => {
  ...
  LoaderErrorPlugin.prototype.start = function () {
    try {
      BdApi.UI.showToast(`LevelProgressBar failed to load ${f}`, { type: 'error' });
    } catch (_) {}
    console.error(`[LevelProgressBar] Failed to load ${f}`);
  };
  LoaderErrorPlugin.prototype.stop = function () {};
  return LoaderErrorPlugin;
};

module.exports = _bdLoad('LevelProgressBarMain.js');
