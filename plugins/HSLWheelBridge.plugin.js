...
function findHSLScroller() {
  if (_PluginUtils) {
    return _PluginUtils.querySelectorFallback(HSL_SCROLLER_SELECTORS, {
      cache: _scrollerCache,
      cacheKey: "hsl-scroller",
      ttlMs: 2e3
    });
  }
  const entry = _scrollerCache["hsl-scroller"];
  if (entry && Date.now() - entry.time < 2e3 && entry.el && entry.el.isConnected) {
    return entry.el;
  }
  let scroller = null;
...
module.exports = class HSLWheelBridge {
  ...
  start() {
    ...
    this._toast("HSLWheelBridge active", "success", 2e3);
  }
  stop() {
    this._isStopped = true;
    this._cleanupRuntime();
    this._toast("HSLWheelBridge stopped", "info", 2e3);
  }
  // =========================================================================
